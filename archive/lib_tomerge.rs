        ...
        if *clsid == MMDeviceEnumerator {
            if hr.is_ok() {
                    ...
                    && KEYWORDS.iter().any(|keyword| name.contains(keyword))
                {
                    ...
                    for qi in std::slice::from_raw_parts_mut(presults, dwcount as usize) {
...
struct RedirectCompatAudioClient {
    ...
    buf_len: u32,
    init_params: UnsafeCell<Option<(HANDLE, u16 /* align */)>>,
    dataflow: DeviceDataFlow,
            ...
            unsafe {
                ...
                if *inner_smp != (*pformat).nSamplesPerSec {
                    ...
                    *UnsafeCell::raw_get(&self.min_len) = pminperiodinframes;
                    ...
                    let callback = CreateEventExA(
                        None,
                        None,
                        CREATE_EVENT::default(),
                        (EVENT_MODIFY_STATE | SYNCHRONIZATION_SYNCHRONIZE).0,
                    )?;
                    self.inner.SetEventHandle(callback)?;
        ...
        if !phnsdefaultdeviceperiod.is_null() {
            unsafe {
                *phnsdefaultdeviceperiod = calculate_period(
                    ...
                    if target_cfg.target_buffer_dur_ms != 0 {
                        calculate_buffer(
                            unsafe { *pformat }.nSamplesPerSec,
                            pfundamentalperiodinframes,
                            target_cfg.target_buffer_dur_ms,
...
fn callback(
    ...
    loop {
        ...
        debug!("callback! data in mid-buffer:{}", read_len);
        debug!("callback! data should write:{}", write_len);
        if write_len == 0 {
            warn!("mid-buffer has been emptied, underflow may happen!");
            continue;
        }
        let slice_ptr = unsafe { inner.GetBuffer(write_len as u32).unwrap() };
        let slice_len = write_len * align;
        let slice = unsafe { std::slice::from_raw_parts_mut(slice_ptr, slice_len) };
        let replaced = dwflag.load(std::sync::atomic::Ordering::Acquire);
        let write_len = buffer.pop_slice(slice) / align;
        padding.store(
            (buffer.occupied_len() / align) as u32,
...
impl RedirectAudioRenderClient {
    fn new(
        ...
        let buffer: HeapRb<u8> = HeapRb::new(buffer_len as usize * align as usize);
        // let buffer = RawAudioBuffer::new(align, buffer_len);
        let (prod, cons) = buffer.split();
        let thread_padding = padding.clone();
        let stop_flag: Arc<AtomicBool> = AtomicBool::new(false).into();
        let flag: Arc<AtomicU32> = AtomicU32::default().into();
        let clientflag = flag.clone();
        let thread_stop_flag = stop_flag.clone();
        let h = handle.0.expose_provenance();

        let thread = spawn(move || {
            let handle = HANDLE(std::ptr::with_exposed_provenance_mut::<c_void>(h));
            callback(
                handle,
                thread_padding,
                cons,
...
impl IAudioRenderClient_Impl for RedirectAudioRenderClient_Impl {
    ...
    fn ReleaseBuffer(&self, numframeswritten: u32, dwflags: u32) -> windows_core::Result<()> {
        ...
        let written_len = if !self.trick.load(std::sync::atomic::Ordering::Acquire) {
            numframeswritten as usize
        } else {
            280
        };
        let slice = unsafe { &(&(*self.temp_buffer.get()))[0..(written_len * self.align)] };
        let buffer = unsafe { &mut *self.buffer.get() };
        let written_len = buffer.push_slice(slice) / self.align;
        self.dwflag
            .store(dwflags, std::sync::atomic::Ordering::Release);
        debug!(
            "dwflag: {dwflags}, silent: {}",
            dwflags & AUDCLNT_BUFFERFLAGS_SILENT.0 as u32
        );
        // self.padding.store(
        //     (buffer.occupied_len() / self.align) as u32,
        //     std::sync::atomic::Ordering::Release,
        // );
