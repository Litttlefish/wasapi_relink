...
struct RedirectCompatAudioClient {
    ...
    dataflow: DeviceDataFlow,
    trick: UnsafeCell<bool>,
    hooker_pad: UnsafeCell<u32>,
}
...
impl IAudioClient_Impl for RedirectCompatAudioClient_Impl {
    fn Initialize(
        ...
        if sharemode == AUDCLNT_SHAREMODE_SHARED {
            ...
            unsafe {
                ...
                *(&mut *self.hooker_pad.get()) =
                    self.inner.GetBufferSize()? - self.hooker.GetBufferSize()?;
                Ok(())
            }
        } else {
            unsafe {
    ...
    fn GetCurrentPadding(&self) -> windows_core::Result<u32> {
        ...
        if !(unsafe { *self.trick.get() }) {
            unsafe { self.inner.GetCurrentPadding() }
        } else {
            Ok(unsafe { *self.hooker_pad.get() })
        }
    }
