...
use windows::Win32::System::Threading::{
    IRtwqAsyncCallback, IRtwqAsyncCallback_Impl, IRtwqAsyncResult, RtwqCreateAsyncResult,
    RtwqLockSharedWorkQueue, RtwqPutWaitingWorkItem, RtwqShutdown, RtwqStartup,
    RtwqUnlockWorkQueue,
};
...
static ORIGINAL_COCREATE: OnceLock<FnCoCreateInstance> = OnceLock::new();
static ORIGINAL_COCREATE_EX: OnceLock<FnCoCreateInstanceEx> = OnceLock::new();
...
unsafe extern "system" fn hooked_cocreateinstance(
    ...
    unsafe {
        let ret =
            ORIGINAL_COCREATE.get().unwrap_unchecked()(rclsid, p_outer, dwcls_context, riid, ppv);
...
unsafe extern "system" fn hooked_cocreateinstanceex(
    ...
    unsafe {
        let hr = ORIGINAL_COCREATE_EX.get().unwrap_unchecked()(
            clsid,
            punkouter,
            dwclsctx,
            pserverinfo,
...
impl Shared3Info {
    fn init(inner: &IAudioClient3, info: &RedirectClientInfo) -> Self {
        ...
        unsafe {
            CoTaskMemFree(Some(pformat.cast()));
        }
        ...
        match iid {
            IAudioRenderClient::IID => {
                if let Some(client) = self.outer.get() {
                    ...
                    unsafe {
                        RtwqLockSharedWorkQueue(AUDIO_TASK, 1, &mut _task_id, &mut thread_id)?
                    };
                    ...
                    let callback = RedirectRingbufThread {
                        ...
                        event_handle: unsafe { Owned::new(event_handle) },
...
fn init() -> std::result::Result<(), HookError> {
    ORIGINAL_COCREATE
        .set(unsafe {
            transmute::<*mut c_void, FnCoCreateInstance>(
                create_hook_api_ex(
                    "ole32",
                    "CoCreateInstance",
                    hooked_cocreateinstance as *mut c_void,
                )?
                .0,
            )
        })
        .unwrap();
    ORIGINAL_COCREATE_EX
        .set(unsafe {
            transmute::<*mut c_void, FnCoCreateInstanceEx>(
                create_hook_api_ex(
                    "ole32",
                    "CoCreateInstanceEx",
                    hooked_cocreateinstanceex as *mut c_void,
                )?
                .0,
            )
        })
...
unsafe extern "system" fn DllMain(_hinst: HANDLE, reason: u32, _reserved: *mut c_void) -> BOOL {
    match reason {
        1 => {
            ...
            initialize()
                .and_then(|_| init())
                .and_then(|_| enable_hook(ALL_HOOKS))
                .is_ok()
        }
        0 => disable_hook(ALL_HOOKS).and_then(|_| uninitialize()).is_ok(),
        _ => true,
    }
    .into()
}
