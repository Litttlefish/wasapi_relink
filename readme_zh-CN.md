...
本工具完全依赖于你的音频驱动程序。配置中的 `target_period_hus` 是一个***请求值***，而非*强制命令*。
...
- 如果程序不使用事件回调，`wasapi_relink` 会注入 `AUDCLNT_STREAMFLAGS_EVENTCALLBACK` 并维护一个专属的高优先级消费线程(基于RTWQ API)，在该线程上等待硬件回调并从环形缓冲区中转移数据到通过实际Client创建的`IAudioRenderClient`，并按照配置的缓冲区汇报长度操作。
...
**结果：** 程序只会看到一个大缓冲的 WASAPI 客户端，与引擎的真实时序和缓冲区完全隔离，即使自身的周期无法兼容小硬件缓冲（基于sleep的轮询、固定大小混音块等）也能正常工作。
...
- `[playback]`/`[capture]`: 分别配置输出和输入。
  ...
  - `target_period_hus` (u32): 所有低延迟流的目标周期大小，单位为 **0.1 毫秒**。如果设置过低或未指定，将默认为驱动的最小值。**除非遇到音频爆音，否则通常不应设置此值。**

  - `target_buffer_len.<samplerate>` (u32): 低延迟流的目标尺寸，单位为**音频帧** (而不是采样点数)。例如，256代表256帧（双声道下为512采样点）。未指定时使用完整缓冲区长度。
...
1. **增加缓冲区：** 略微增加 `target_period_hus` 。如果驱动的最小值是 2ms (即 `20`)，请尝试 `30` (3ms) 或 `40` (4ms)，直到爆音消失。
...
**原因：** 兼容模式里，`compat_buffer_dur_hns` 会一对一映射到目前使用的采样率上。如果你切换了Windows的采样率，但没有在配置里预先设置，它就会使用默认值，这对新采样率来说可能不够用。
...
compat_buffer_dur_hns.96000 = 250000  # 为 96kHz 指定一个值
```
