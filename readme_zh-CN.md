...
2. **引擎侧效果：** 根据[**微软的低延迟音频文档**](https://learn.microsoft.com/en-us/windows-hardware/drivers/audio/low-latency-audio#faq)，有程序通过`IAudioClient3`申请低延迟缓冲区时，音频引擎会把该端点上的所有共享流都切换到那个周期。通过维持这个低延迟实例，`wasapi_relink`强制引擎跑在小周期上，即使程序本身用的是大缓冲区。

3. **预填充欺骗：** 在`IAudioClient::Start`前，许多应用会写入“静默”预填充数据。`wasapi_relink`会通过包装的`IAudioRenderClient`截获该过程，并向底层硬件写入一个小得多（与实际引擎周期匹配）的预填充块。
...
Windows 音频引擎对时序 _极其_ 敏感。
 ...
 - 只有在您明确知道自己在做什么，并主动为程序进行调试时，才应该使用 `Debug`/`Trace`。
...
### 尺寸错位：为什么需要环缓模式
即使你成功强制音频引擎使用低延迟周期（比如128帧，48kHz下大概就是2.67ms），程序和Windows之间还是会有一些基础上的错位。
...
**轮询型：** 如果缓冲区比轮询周期短，游戏就无法输出声音，导致严重的下溢。

**固定混音块型：** 如果缓冲区比混音块尺寸小，音频引擎可能会无法启动。

`wasapi_relink`的环形缓冲区：

环缓模式会在音频引擎和程序间插入一个高性能的软件解耦层：

**引擎侧：** `wasapi_relink`会完全接管 Windows 的回调。它会在每128帧后准时醒来，安静地从环形缓冲区中转移相应数量的数据进去，硬件不会发现游戏的混乱操作。
...
3. 当应用在这个组件上请求设备时， `wasapi_relink` 返回一个包装的 `IMMDevice` 。
    ...
    3.3. **环缓模式：** 当应用在包装的 `IAudioClient` 上调用 `Initialize` 时，将会强制启用事件回调，启用低延迟流，根据应用行为选择使用注入还是倒挂模式，并创建消费线程。
